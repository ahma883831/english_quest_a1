...
// ⚠️ کلید API از پنل GapGPT (gapgpt.app):
const String gapgptApiKey = 'sk-XpECGZ3XoT6leZCfqJMMmGCdvzuff5UBdEWSMUyRkRtiQVQq';
...
class FriendMessage {
  ...
  final String text;
  final int time;
  FriendMessage({required this.sender, required this.text, required this.time});
}
...
class _FriendChatPageState extends State<FriendChatPage> {
  ...
  void _joinRoom(String code) {
    ...
    setState(() {
      roomCode = trimmed;
      messages.clear();
      messages.addAll(AppStorage.loadFriendChat(trimmed));
    });
    AppStorage.saveRoomCode(trimmed);
    _startPolling();
  }
  ...
  Future<void> _poll() async {
    ...
    try {
      ...
      for (final line in lines) {
        ...
        String sender = 'دوست';
        String text = data['message'] as String? ?? '';
        try {
          final payload = jsonDecode(text) as Map<String, dynamic>;
          sender = payload['sender'] ?? sender;
          text = payload['text'] ?? text;
        } catch (_) {
          // not our JSON format (e.g. sent from the ntfy app directly) — show as-is
        }

        final exists = messages.any((m) => m.time == time && m.text == text && m.sender == sender);
        if (!exists) {
          messages.add(FriendMessage(sender: sender, text: text, time: time));
          added = true;
        }
      }
  ...
  Widget build(BuildContext context) {
            ...
            child: Padding(
              ...
              child: Directionality(
                ...
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  const SizedBox(height: 16),
                  const Text('یه اسم اتاق مشترک بساز', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 8),
                  const Text(
                    'همین اسم رو به رفیقت بده تا اونم وارد همون اتاق بشه. یه اسم خاص و غیرتکراری انتخاب کنید (مثلا: ali_sara_secret_482) چون هرکسی این اسم رو بدونه می‌تونه پیام‌ها رو ببینه.',
                  ...
                  SizedBox(
                    width: double.infinity,
                    child: ElevatedButton(
                      onPressed: () => _joinRoom(roomCtrl.text),
                      style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 14)),
    ...
    return Scaffold(
      appBar: AppBar(
        title: Directionality(textDirection: TextDirection.rtl, child: Text('چت • $roomCode')),
        backgroundColor: Colors.transparent,
        actions: [
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return Align(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            color: isMe ? const Color(0xFF00C8FF).withOpacity(.18) : Colors.white.withOpacity(.06),
                            borderRadius: BorderRadius.circular(16),
                            border: Border.all(color: isMe ? const Color(0xFF00E5FF).withOpacity(.4) : Colors.white24),
                          ),
                          child: Directionality(
                            textDirection: TextDirection.rtl,
                            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
