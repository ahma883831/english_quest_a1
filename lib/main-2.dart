        ...
        child: Padding(
          ...
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            const Directionality(
              textDirection: TextDirection.rtl,
              child: Text('تبریک! درس رو کامل کردی', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
            ),
            const SizedBox(height: 10),
            Text('Score: $score / ${widget.lesson.questions.length}', style: const TextStyle(color: Color(0xFF00E5FF), fontSize: 16)),
            const SizedBox(height: 26),
            ElevatedButton.icon(
              onPressed: () {
                widget.onReward();
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('🎉 +100 XP • +20 Coins')));
              },
              icon: const Icon(Icons.bolt_rounded),
              label: const Padding(padding: EdgeInsets.all(10), child: Text('دریافت جایزه', style: TextStyle(fontWeight: FontWeight.bold))),
            ),
          ]),
        ),
      );
