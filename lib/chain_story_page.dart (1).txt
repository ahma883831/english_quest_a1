// ============================================================
// CHAIN STORY PAGE
// Pick one or more lessons, then get a random set of 3-5 words
// pooled from all of them, and write a short 2-3 sentence story
// that uses every one of those words. Checks (simple
// substring/whole-word match, case-insensitive) whether each
// target word appears in what they wrote, and gives per-word
// feedback.
// ============================================================
import 'dart:math';
...
class _ChainStoryPageState extends State<ChainStoryPage> {
  ...
  void _pickWords() {
    final rnd = Random();
    final pool = currentLessons.expand((l) => l.words).toList();
    pool.shuffle(rnd);
