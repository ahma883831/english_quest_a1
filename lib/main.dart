...
class AppStorage {
  ...
  static Future<void> saveWritingText(int topicId, String text) async =>
      _prefs.setString('writing_$topicId', text);
  static String loadWritingText(int topicId) => _prefs.getString('writing_$topicId') ?? '';
  static int writingFilledCount(int totalTopics) {
    var count = 0;
    for (var i = 1; i <= totalTopics; i++) {
      if (loadWritingText(i).trim().isNotEmpty) count++;
    }
    return count;
  }
...
class AppState extends ChangeNotifier {
  ...
  // activity is one of: 'quiz', 'sentences'
  ...
  bool isLessonFullyComplete(int lessonId) =>
      isActivityComplete(lessonId, 'quiz') && isActivityComplete(lessonId, 'sentences');
...
class _GlobalNotebookButtonState extends State<_GlobalNotebookButton> {
  Offset? position;
  Color fabColor = const Color(0xFF00E5FF);
  String fabTarget = 'practice_hub';

  static const Map<String, String> _targets = {
    'practice_hub': 'تمرین',
    'writing_hub': 'نوشتن',
    'speed_duel': 'دوئل سریع',
  };
  static const Map<String, String> _targetEmoji = {
    'practice_hub': '🏋️',
    'writing_hub': '✍️',
    'speed_duel': '⚡',
  };
  ...
  Future<void> _loadPrefs() async {
    ...
    final colorVal = prefs.getInt('fab_color');
    final target = prefs.getString('fab_target');
    setState(() {
      if (colorVal != null) fabColor = Color(colorVal);
      if (target != null && _targets.containsKey(target)) fabTarget = target;
    });
  }

  Future<void> _savePrefs() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt('fab_color', fabColor.value);
    await prefs.setString('fab_target', fabTarget);
  }

  Widget _destinationFor(String target) {
    switch (target) {
      case 'writing_hub':
        return const WritingHubPage();
      case 'speed_duel':
        return const LessonPickerForDuelPage();
      default:
        return const PracticeHubPage();
    }
  }
        ...
        builder: (ctx, setSheetState) => Directionality(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                const Text('شخصی‌سازی آیکون شناور', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white)),
                const SizedBox(height: 16),
                const Text('رنگ', style: TextStyle(color: Colors.white54, fontSize: 13)),
                const SizedBox(height: 8),
                Wrap(
                  spacing: 10,
                  children: presetColors.map((c) {
                    final selected = c.value == fabColor.value;
                ...
                const Text('وقتی روش می‌زنی، ببرتم به:', style: TextStyle(color: Colors.white54, fontSize: 13)),
                const SizedBox(height: 8),
                ..._targets.entries.map((e) => RadioListTile<String>(
                      value: e.key,
                      groupValue: fabTarget,
                      activeColor: fabColor,
                      title: Text('${_targetEmoji[e.key]}  ${e.value}', style: const TextStyle(color: Colors.white)),
                      onChanged: (v) {
                        if (v == null) return;
                        setSheetState(() {});
                        setState(() => fabTarget = v);
                        _savePrefs();
                      },
                    )),
              ],
      ...
      child: GestureDetector(
        ...
        onTap: () {
          appNavigatorKey.currentState?.push(
            MaterialPageRoute(builder: (_) => _destinationFor(fabTarget)),
          );
        },
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: fabColor.withOpacity(.7), width: 1.5),
            boxShadow: const [BoxShadow(color: Colors.black54, blurRadius: 8, offset: Offset(0, 3))],
          ),
          child: Center(child: Text(_targetEmoji[fabTarget] ?? '📓', style: const TextStyle(fontSize: 24))),
        ),
      ),
    );
  }
          ...
          child: Directionality(
            ...
            child: ListView(
              ...
              children: [
                ...
                _drawerItem(context, Icons.edit_note_rounded, 'نوشتن', () {
                  Navigator.pop(context);
                  Navigator.push(context, MaterialPageRoute(builder: (_) => const WritingHubPage()));
                }),
                _drawerItem(context, Icons.auto_stories_rounded, 'داستان زنجیره‌ای', () async {
                  Navigator.pop(context);
                  ...
                  if (picked != null && picked.isNotEmpty && context.mounted) {
                    Navigator.push(context, MaterialPageRoute(builder: (_) => ChainStoryPage(initialLessons: picked)));
                  }
                }),
                ...
                _drawerItem(context, Icons.headphones_rounded, 'تمرین شنیداری', () async {
                  Navigator.pop(context);
                  final picked = await Navigator.push<List<Lesson>>(
                    context,
                    MaterialPageRoute(builder: (_) => const MultiLessonPickerPage()),
                  ...
                  if (picked != null && picked.isNotEmpty && context.mounted) {
                    Navigator.push(context, MaterialPageRoute(builder: (_) => ListeningPracticePage(initialLessons: picked)));
                  }
                }),
        ...
        GridView.count(
          ...
          children: [
            ...
            MenuCard(
              ...
              titleFa: 'نوشتن',
              subtitleFa: '${AppStorage.writingFilledCount(writingTopics.length)}/${writingTopics.length} موضوع',
              color: const Color(0xFF00E5A0),
              onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const WritingHubPage())),
            ),
            MenuCard(
...
class AdventureMapPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final writingDone = AppStorage.writingFilledCount(writingTopics.length);
    ...
    final nodes = <_MapNodeData>[
      ...
      _MapNodeData(emoji: '✍️', titleFa: 'نوشتن', subtitleFa: '$writingDone/${writingTopics.length}', current: writingDone, total: writingTopics.length, align: 0.72, decorative: false, locked: false,
          onTap: (ctx) => Navigator.push(ctx, MaterialPageRoute(builder: (_) => const WritingHubPage()))),
          ...
          child: Container(
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const SizedBox(height: 6),
                const Text('examples • quiz', style: TextStyle(color: Colors.white54, fontSize: 11)),
              ])),
...
class _LessonPageState extends State<LessonPage> {
        ...
        body: Column(children: [
          SizedBox(
            ...
            child: SingleChildScrollView(
              ...
              child: Row(children: [
                _TabButton('Learn', 0),
                _TabButton('Examples', 1),
                _TabButton('Practice', 2),
                _TabButton('Quiz', 3),
              ]),
            ),
          ),
  ...
  Widget _content() {
    switch (tab) {
      ...
      case 1: return _examples();
      case 2: return _practice();
      default: return _quiz();
    }
  }

  Widget _practice() => SingleChildScrollView(
        ...
        child: Column(children: [
          _PracticeOptionCard(
            ...
            color: const Color(0xFF7C4DFF),
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => SentenceBuilderPage(lessonsFilter: [widget.lesson]))),
          ),
          const SizedBox(height: 16),
          _PracticeOptionCard(
            emoji: '🎤',
            titleFa: 'تمرین تلفظ جملات',
          ...
          _PracticeOptionCard(
            ...
            subtitleFa: 'جمله رو بشنو و معنیش رو حدس بزن',
            color: const Color(0xFF29B6F6),
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ListeningPracticePage(initialLessons: [widget.lesson]))),
          ),
          const SizedBox(height: 16),
          _PracticeOptionCard(
            emoji: '📖',
            titleFa: 'داستان زنجیره‌ای این درس',
            subtitleFa: 'با ساختار گرامری این درس یک جمله/داستان بساز',
            color: const Color(0xFF66BB6A),
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ChainStoryPage(initialLessons: [widget.lesson]))),
          ),
          const SizedBox(height: 16),
          _PracticeOptionCard(
            emoji: '⚡',
            titleFa: 'دوئل سریع همین درس',
            subtitleFa: '۶۰ ثانیه فقط با سؤالات همین درس مسابقه بده',
            color: const Color(0xFFEF5350),
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => SpeedDuelGamePage(lessons: [widget.lesson]))),
          ),
          const SizedBox(height: 16),
          _PracticeOptionCard(
            emoji: '✍️',
            titleFa: 'نوشتن آزاد',
            subtitleFa: 'با موضوعات مختلف جمله بساز و تمرین کن',
  ...
  Widget _exampleTile(String raw) {
    final parts = raw.split(' — ');
  ...
  Widget _exampleSection(String titleFa, List<String> items) {
    if (items.isEmpty) return const SizedBox.shrink();
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Directionality(
        textDirection: TextDirection.rtl,
        child: Text(titleFa, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: Color(0xFF00E5FF))),
      ),
      const SizedBox(height: 8),
      ...items.map(_exampleTile),
  ...
  Widget _examples() => ListView(
        ...
        padding: const EdgeInsets.all(18),
        children: [
          _exampleSection('✅ جملات مثبت', widget.lesson.examples),
          _exampleSection('🚫 جملات منفی', widget.lesson.negativeExamples),
          _exampleSection('❓ جملات سؤالی', widget.lesson.questionExamples),
        ],
      );
...
class ProgressPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final totalLessons = lessons.length;
    final completedLessons = (xp ~/ 100).clamp(0, totalLessons);
    final sentencesPerLesson = lessons.isNotEmpty
        ? lessons.first.examples.length + lessons.first.negativeExamples.length + lessons.first.questionExamples.length
        : 0;
    final learnedSentences = completedLessons * sentencesPerLesson;
    final totalSentences = totalLessons * sentencesPerLesson;
        ...
        Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
          ...
          Expanded(
            child: Column(children: [
              ...
              const SizedBox(height: 10),
              _StatMiniCard(icon: Icons.style_rounded, label: 'جملات آموخته‌شده', value: '$learnedSentences/$totalSentences'),
            ]),
          ),
        ]),
        const SizedBox(height: 24),
...
// Generic single-lesson picker — pops with the chosen Lesson.
// Used by every practice tool that needs "which lesson?" before
// starting, and by the in-page "🔀 تغییر درس" buttons.
class SingleLessonPickerPage extends StatelessWidget {
  const SingleLessonPickerPage({super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colors.transparent,
        title: const Directionality(textDirection: TextDirection.rtl, child: Text('انتخاب درس')),
      ),
      body: SafeArea(
        child: Directionality(
          textDirection: TextDirection.rtl,
          child: ListView.builder(
            ...
            itemBuilder: (_, i) {
              ...
              return Card(
                ...
                child: ListTile(
                  title: Text(l.subtitle, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                  subtitle: Text(l.title, style: const TextStyle(color: Colors.white38)),
                  trailing: const Icon(Icons.chevron_left_rounded, color: Color(0xFF00E5FF)),
...
// Generic MULTI-lesson picker — pops with a List<Lesson> (one or
// more). Used by every practice tool so you can mix words from
// several lessons into one bigger, more random session.
class MultiLessonPickerPage extends StatefulWidget {
  const MultiLessonPickerPage({super.key});
  @override
...
class PracticeHubPage extends StatelessWidget {
  ...
  Future<void> _pickMultiThen(BuildContext context, Widget Function(List<Lesson>) builder) async {
    final picked = await Navigator.push<List<Lesson>>(
      context,
      MaterialPageRoute(builder: (_) => const MultiLessonPickerPage()),
    );
    if (picked != null && picked.isNotEmpty) {
      Navigator.push(context, MaterialPageRoute(builder: (_) => builder(picked)));
    }
  }

  Future<void> _pickThen(BuildContext context, Widget Function(Lesson) builder) async {
    final picked = await Navigator.push<Lesson>(
      context,
      MaterialPageRoute(builder: (_) => const SingleLessonPickerPage()),
    );
    if (picked != null) {
      Navigator.push(context, MaterialPageRoute(builder: (_) => builder(picked)));
    }
          ...
          SingleChildScrollView(
            ...
            child: Column(children: [
              _PracticeOptionCard(
                emoji: '📝',
                titleFa: 'تمرین جملات',
                subtitleFa: 'جمله‌ی انگلیسی رو از روی معنی بساز (۳۰ سؤال تصادفی)',
                color: const Color(0xFF7C4DFF),
                onTap: () => _pickMultiThen(context, (ls) => SentenceBuilderPage(lessonsFilter: ls)),
              ),
              const SizedBox(height: 16),
              _PracticeOptionCard(
                emoji: '🗣️',
                titleFa: 'تمرین تلفظ',
                subtitleFa: 'بگو و درصد دقت تلفظت رو ببین',
                color: const Color(0xFF26C6DA),
                onTap: () => _pickThen(context, (l) => PronunciationPracticePage(lesson: l)),
              ),
              const SizedBox(height: 16),
              _PracticeOptionCard(
                emoji: '🎧',
                titleFa: 'تمرین شنیداری',
                subtitleFa: 'جمله رو بشنو و معنیش رو حدس بزن (۳۰ سؤال تصادفی)',
                color: const Color(0xFF29B6F6),
                onTap: () => _pickMultiThen(context, (ls) => ListeningPracticePage(initialLessons: ls)),
              ),
              const SizedBox(height: 16),
              _PracticeOptionCard(
                emoji: '📖',
                titleFa: 'داستان زنجیره‌ای',
                subtitleFa: 'با یک ساختار گرامری تصادفی جمله/داستان بساز',
                color: const Color(0xFF66BB6A),
                onTap: () => _pickMultiThen(context, (ls) => ChainStoryPage(initialLessons: ls)),
              ),
              const SizedBox(height: 16),
              _PracticeOptionCard(
                emoji: '⚡',
              ...
              _PracticeOptionCard(
                emoji: '✍️',
                titleFa: 'نوشتن آزاد',
                subtitleFa: '۳۰ موضوع برای نوشتن جمله‌های خودت',
                color: const Color(0xFFAB47BC),
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const WritingHubPage())),
              ),
            ]),
...
class SentenceBuilderPage extends StatefulWidget {
  final List<Lesson>? lessonsFilter;
  const SentenceBuilderPage({super.key, this.lessonsFilter});
  @override
...
class _SentenceBuilderPageState extends State<SentenceBuilderPage> {
  ...
  void initState() {
    ...
    currentLessons = widget.lessonsFilter ?? [];
    _rebuildSentences();
  }

  static const int _kRounds = 30;

  void _rebuildSentences() {
    final allExamples = currentLessons.isNotEmpty
        ? (currentLessons.expand((l) => l.examples).toList()..shuffle())
        : (lessons.expand((l) => l.examples).toList()..shuffle());
    final limited = allExamples.length > _kRounds ? allExamples.sublist(0, _kRounds) : allExamples;
    englishSentences = [];
    persianMeanings = [];
  ...
  void _next() {
    if (round + 1 >= englishSentences.length) {
      if (currentLessons.length == 1) {
        AppState.instance.markActivityComplete(currentLessons.first.id, 'sentences');
      }
      if (englishSentences.isNotEmpty) AppState.instance.addReward(20, 5);
      Navigator.pop(context);
      return;
    }
    setState(() => round++);
