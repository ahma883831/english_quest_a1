...
class AppState extends ChangeNotifier {
  ...
  // Lesson 1 (the first one) is always unlocked; every later lesson
  // unlocks once the previous lesson's quiz + word practice + sentence practice are all done.
  bool isLessonUnlocked(int lessonId) {
    if (lessonId <= lessons.first.id) return true;
    final idx = lessons.indexWhere((l) => l.id == lessonId);
    if (idx <= 0) return true;
    final prevLesson = lessons[idx - 1];
    return isLessonFullyComplete(prevLesson.id);
  }
}
