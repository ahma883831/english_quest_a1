...
class _GlobalNotebookButtonState extends State<_GlobalNotebookButton> {
  ...
  String fabTarget = 'mistake_notebook';

  static const Map<String, String> _targets = {
    'mistake_notebook': 'دفترچه‌ی اشتباهات',
    'practice_hub': 'تمرین',
    'words_bank': 'بانک کلمات',
  ...
  Widget _destinationFor(String target) {
    switch (target) {
      ...
      case 'words_bank':
        return const WordsBankPage();
      case 'speed_duel':
        return const LessonPickerForDuelPage();
      default:
        return const MistakeNotebookPage();
    }
  }
          ...
          child: Directionality(
            ...
            child: ListView(
              ...
              children: [
                ...
                _drawerItem(context, Icons.keyboard_rounded, 'ترجمه‌ی معکوس', () async {
                  Navigator.pop(context);
                  final picked = await Navigator.push<List<Lesson>>(
                    context,
                    MaterialPageRoute(builder: (_) => const MultiLessonPickerPage()),
                  ...
                  if (picked != null && picked.isNotEmpty && context.mounted) {
                    Navigator.push(context, MaterialPageRoute(builder: (_) => ReverseRecallPage(initialLessons: picked)));
                  }
                }),
  ...
  Widget _practice() => SingleChildScrollView(
        ...
        child: Column(children: [
          _PracticeOptionCard(
            ...
            color: const Color(0xFF00E5A0),
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => WordBuilderPage(lessonsFilter: [widget.lesson]))),
          ),
          const SizedBox(height: 16),
          _PracticeOptionCard(
            emoji: '📝',
          ...
          _PracticeOptionCard(
            ...
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ReverseRecallPage(initialLessons: [widget.lesson]))),
          ),
          const SizedBox(height: 16),
          _PracticeOptionCard(
            emoji: '📖',
            titleFa: 'داستان زنجیره‌ای این درس',
            subtitleFa: 'با چند کلمه‌ی این درس یک داستان کوتاه بساز',
          ...
          _PracticeOptionCard(
            ...
            subtitleFa: 'مرور کلماتی که قبلاً اشتباه زدی (همه‌ی درس‌ها)',
                    ...
                    : () {
                        setState(() {
                          ...
                          if (opt == q.answer) {
                            ...
                            wrongCount++;
                            final match = widget.lesson.words.firstWhere(
                              (w) => w.word == q.answer || w.meaning == q.answer,
                              orElse: () => const Word('', '', '', ''),
                            );
                            if (match.word.isNotEmpty) MistakeService.recordMistake(match);
                          }
                        });
                      },
                child: Container(
          ...
          SingleChildScrollView(
            ...
            child: Column(children: [
              _PracticeOptionCard(
                emoji: '🔤',
                titleFa: 'تمرین کلمات',
                subtitleFa: 'بازی ساخت کلمه از روی معنی و تلفظ (۳۰ سؤال تصادفی)',
                color: const Color(0xFF00E5A0),
                onTap: () => _pickMultiThen(context, (ls) => WordBuilderPage(lessonsFilter: ls)),
              ),
              const SizedBox(height: 16),
              _PracticeOptionCard(
                emoji: '📝',
              ...
              _PracticeOptionCard(
                ...
                subtitleFa: 'فقط صدا رو بشنو و معنی رو انتخاب کن (۳۰ سؤال تصادفی)',
                color: const Color(0xFF29B6F6),
              ...
              _PracticeOptionCard(
                ...
                subtitleFa: 'معنی فارسی رو ببین، کلمه‌ی انگلیسی رو تایپ کن (۳۰ سؤال تصادفی)',
                color: const Color(0xFFFFB300),
                onTap: () => _pickMultiThen(context, (ls) => ReverseRecallPage(initialLessons: ls)),
              ),
              const SizedBox(height: 16),
              _PracticeOptionCard(
                emoji: '📖',
...
class WordBuilderPage extends StatefulWidget {
  ...
  const WordBuilderPage({super.key, this.lessonsFilter});
  @override
...
class _WordBuilderPageState extends State<WordBuilderPage> {
  ...
  void _rebuildPool() {
    pool = currentLessons.isNotEmpty
        ? (currentLessons.expand((l) => l.words).toList()..shuffle())
        : (lessons.expand((l) => l.words).toList()..shuffle());
    if (pool.length > _kRounds) pool = pool.sublist(0, _kRounds);
    round = 0;
    correctCount = 0;
  ...
  void _next() {
    if (round + 1 >= pool.length) {
      if (currentLessons.length == 1) {
        AppState.instance.markActivityComplete(currentLessons.first.id, 'words');
      }
      if (pool.isNotEmpty) AppState.instance.addReward(20, 5);
      Navigator.pop(context);
      return;
    }
    setState(() => round++);
