            ...
            child: ListView(
              ...
              children: [
                ...
                _drawerItem(context, Icons.menu_book_rounded, 'همه‌ی درس‌ها', () {
                  Navigator.pop(context);
                  Navigator.push(context, MaterialPageRoute(builder: (_) => LessonsListPage(onReward: () {}, xp: xp)));
                }),
        ...
        GridView.count(
          ...
          children: [
            MenuCard(
              ...
              onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => LessonsListPage(onReward: onReward, xp: xp))),
            ),
            MenuCard(
...
class LessonsListPage extends StatelessWidget {
  final VoidCallback onReward;
  final int xp;
  const LessonsListPage({super.key, required this.onReward, required this.xp});

  @override
  Widget build(BuildContext context) {
    final completedLessons = (xp ~/ 100).clamp(0, lessons.length);

    final nodes = <_MapNodeData>[];
    for (int i = 0; i < lessons.length; i++) {
      final lesson = lessons[i];
      final unlocked = i <= completedLessons;
      final align = (i % 2 == 0) ? 0.28 : 0.72;
      nodes.add(_MapNodeData(
        emoji: kLessonIslandEmojis[i % kLessonIslandEmojis.length],
        ...
        decorative: false,
        locked: !unlocked,
        onTap: (ctx) => Navigator.push(ctx, MaterialPageRoute(builder: (_) => LessonPage(lesson: lesson, onReward: onReward))),
      ));
    }
  ...
  Widget build(BuildContext context) {
    ...
    final nodes = <_MapNodeData>[
      ...
      _MapNodeData(emoji: '📘', titleFa: 'گرامر', subtitleFa: '$completedLessons/$totalLessons', current: completedLessons, total: totalLessons, align: 0.28, decorative: false, locked: false,
          onTap: (ctx) => Navigator.push(ctx, MaterialPageRoute(builder: (_) => LessonsListPage(onReward: () {}, xp: xp)))),
...
class _LessonPageState extends State<LessonPage> {
  ...
  Widget _quizComplete() {
    ...
    // Play a distinct sound/voice for success vs. failure, once per screen.
    if (!feedbackPlayed) {
      feedbackPlayed = true;
              ...
              if (passed)
                Container(
                  ...
                  child: const Row(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    SizedBox(width: 8),
                    Text('+50', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w900, color: Colors.amber)),
                  ]),
                ),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: Directionality(
                    textDirection: TextDirection.rtl,
                    child: Text(
                      passed ? (rewardClaimed ? 'جایزه دریافت شد ✅' : 'ادامه') : 'تلاش دوباره',
                      style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
                    ),
                  ),
                ),
...
class _JournalPageState extends State<JournalPage> {
  ...
  void _delete(int index) => setState(() => entries.removeAt(index));
