...
void main() async {
  // Show a readable Persian error screen instead of a blank white/black
  // screen whenever a widget throws — so if this happens again, you can
  // screenshot the actual message instead of just seeing a blank app.
          ...
          child: Padding(
            ...
            child: Directionality(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                const Text('یه خطا پیش اومد', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
  ...
  runZonedGuarded(() async {
    ...
    // Each step is wrapped separately: if one part fails (e.g. notifications
    // or a corrupted saved value), the rest of the app still loads instead
    // of the whole app dying silently on a blank screen.
    try {
      await AppStorage.init();
    } catch (e, st) {
      debugPrint('AppStorage.init failed: $e\n$st');
    }
    try {
    ...
    try {
      ...
      debugPrint('loadProfileInto failed: $e\n$st');
    }
    ...
    try {
      ...
      debugPrint('rescheduleFromSavedSettings failed: $e\n$st');
    }
...
class AppStorage {
  ...
  static List<JournalEntry> loadJournal() {
    ...
    for (final s in raw) {
      try {
        ...
        result.add(JournalEntry(title: m['title'], content: m['content'], date: DateTime.parse(m['date'])));
      } catch (_) {
        // skip a corrupted entry instead of crashing the whole list
      }
    }
    return result;
  }
  ...
  static List<ChatMessage> loadChat() {
    ...
    for (final s in raw) {
      try {
        final m = jsonDecode(s) as Map<String, dynamic>;
        result.add(ChatMessage(m['text'], m['isUser']));
      } catch (_) {}
    }
    return result;
  }
  ...
  static List<FriendMessage> loadFriendChat(String room) {
    ...
    final result = <FriendMessage>[];
    for (final s in raw) {
      try {
        final m = jsonDecode(s) as Map<String, dynamic>;
        result.add(FriendMessage(sender: m['sender'], text: m['text'], time: m['time']));
      } catch (_) {}
    }
    return result;
  }
  ...
  // --- Mistake book (wrong quiz answers, for smart review) ---
  static Future<void> saveMistakes(List<MistakeItem> items) async {
    final encoded = items.map((m) => jsonEncode(m.toJson())).toList();
    await _prefs.setStringList('mistake_book', encoded);
  }

  static List<MistakeItem> loadMistakes() {
    final raw = _prefs.getStringList('mistake_book') ?? [];
    final result = <MistakeItem>[];
    for (final s in raw) {
      try {
        result.add(MistakeItem.fromJson(jsonDecode(s)));
      } catch (_) {}
    }
    return result;
  }
...
// ============================================================
// MISTAKE BOOK — 🔁 دفترچه‌ی اشتباهات + مرور هوشمند
// سوال‌هایی که تو کوییز اشتباه میزنی اینجا جمع میشن تا فقط
// همونا رو مرور کنی، به‌جای تکرار هرچی که از قبل بلدی.
// ============================================================
class MistakeItem {
  final int lessonId;
  final String text;
  ...
  final String answer;
  MistakeItem({required this.lessonId, required this.text, required this.options, required this.answer});

  Map<String, dynamic> toJson() => {'lessonId': lessonId, 'text': text, 'options': options, 'answer': answer};
...
class MistakeBook {
  static List<MistakeItem>? _cache;

  static Future<List<MistakeItem>> getAll() async {
    _cache ??= AppStorage.loadMistakes();
    return _cache!;
  }

  static Future<void> add(int lessonId, Question q) async {
    await getAll();
    final exists = _cache!.any((m) => m.lessonId == lessonId && m.text == q.text);
    if (!exists) {
      _cache!.add(MistakeItem(lessonId: lessonId, text: q.text, options: q.options, answer: q.answer));
      await AppStorage.saveMistakes(_cache!);
    }
  }

  static Future<void> remove(MistakeItem item) async {
    await getAll();
    _cache!.removeWhere((m) => m.lessonId == item.lessonId && m.text == item.text);
    await AppStorage.saveMistakes(_cache!);
  }
}
          ...
          child: Directionality(
            ...
            child: ListView(
              ...
              children: [
                ...
                _drawerItem(context, Icons.repeat_rounded, 'مرور اشتباهات', () {
                  Navigator.pop(context);
                  Navigator.push(context, MaterialPageRoute(builder: (_) => const MistakeReviewPage()));
                }),
...
class _LessonPageState extends State<LessonPage> {
        ...
        appBar: AppBar(
          ...
          actions: [
            Padding(
              ...
              child: IconButton(
                tooltip: 'Interactive World',
                icon: const Icon(Icons.public_rounded, color: Color(0xFF00E5FF)),
                onPressed: () => Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (_) => InteractiveWorldPage(lessonId: widget.lesson.id),
                  ),
                ),
              ),
            ),
          ...
          Padding(
            ...
            child: SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: const Text('🌍 دنیای تعاملی این درس', style: TextStyle(fontWeight: FontWeight.w900)),
                style: ElevatedButton.styleFrom(
                  padding: const EdgeInsets.symmetric(vertical: 13),
                  backgroundColor: const Color(0xFF00C8FF).withOpacity(.13),
                  foregroundColor: const Color(0xFF00E5FF),
                  side: BorderSide(color: const Color(0xFF00E5FF).withOpacity(.35)),
  ...
  Widget _practice() => Padding(
        ...
        child: Column(children: [
          ...
          _PracticeOptionCard(
            emoji: '🔄',
            titleFa: 'ترجمه‌ی معکوس این درس',
            subtitleFa: 'فارسی رو خودت به انگلیسی تایپ کن',
            color: const Color(0xFFFFC107),
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ReverseTranslationPage(lesson: widget.lesson))),
          ),
          const SizedBox(height: 16),
          _PracticeOptionCard(
            emoji: '🎧',
            titleFa: 'تمرین شنیداری این درس',
            subtitleFa: 'جمله رو گوش بده و کلمه‌ی جا افتاده رو پیدا کن',
            color: const Color(0xFF00C8FF),
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ListeningGapFillPage(lesson: widget.lesson))),
          ),
        ]),
      );
              ...
              return GestureDetector(
                onTap: selectedAnswer != null
                    ...
                    : () {
                        ...
                        if (opt != q.answer) {
                          MistakeBook.add(widget.lesson.id, q);
                        }
                      },
                child: Container(
...
// Shared helper so any page (Tutor, Journal AI-check, etc.) can call the same API
// without duplicating the request logic.
Future<String> askGapGPT({required String systemPrompt, required String userMessage}) async {
  ...
  throw Exception('خطا در دریافت پاسخ (کد ${response.statusCode}): ${response.body}');
}
        ...
        body: Stack(children: [
          ...
          SingleChildScrollView(
            ...
            child: Column(children: [
              ...
              _PracticeOptionCard(
                emoji: '🔁',
                titleFa: 'مرور اشتباهات',
                subtitleFa: 'فقط سوال‌هایی که تو کوییز غلط زدی',
                color: const Color(0xFFFF5C7A),
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const MistakeReviewPage())),
              ),
              const SizedBox(height: 16),
              _PracticeOptionCard(
                emoji: '🔄',
                ...
                subtitleFa: 'فارسی رو خودت به انگلیسی تایپ کن',
                color: const Color(0xFFFFC107),
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const ReverseTranslationPage())),
              ),
              const SizedBox(height: 16),
              _PracticeOptionCard(
                emoji: '🎧',
                ...
                subtitleFa: 'جمله رو گوش بده و کلمه‌ی جا افتاده رو پیدا کن',
                color: const Color(0xFF00C8FF),
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const ListeningGapFillPage())),
              ),
            ]),
          ),
        ]),
...
class _JournalPageState extends State<JournalPage> {
  ...
  Future<void> _checkWithAI(int index) async {
    setState(() => checking[index] = true);
    try {
      final reply = await askGapGPT(
        systemPrompt:
            'You are a friendly Persian-speaking English writing coach for an A1 (beginner) student. '
            'The student will send you a short text they wrote in English. Point out grammar or spelling '
            'mistakes, explain each one briefly in Persian, and show the corrected sentence. Keep it short, '
            'simple, and encouraging. Respond in Persian (you can quote the English words/corrections as-is).',
        userMessage: entries[index].content,
      );
      setState(() => feedback[index] = reply);
    } catch (e) {
      setState(() => feedback[index] = 'خطا در بررسی: $e');
    } finally {
      setState(() => checking[index] = false);
    }
  }
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Align(
                          ...
                          child: TextButton.icon(
                            onPressed: (checking[i] ?? false) ? null : () => _checkWithAI(i),
                            icon: (checking[i] ?? false)
                                ? const SizedBox(width: 14, height: 14, child: CircularProgressIndicator(strokeWidth: 2))
                                : const Icon(Icons.auto_awesome_rounded, size: 16, color: Color(0xFF00E5FF)),
                            label: Directionality(
                              textDirection: TextDirection.rtl,
                              child: Text(
                                (checking[i] ?? false) ? 'در حال بررسی...' : 'بررسی با هوش مصنوعی',
                        ...
                        if (feedback[i] != null) ...[
                          ...
                          Container(
                            ...
                            child: Directionality(
                              textDirection: TextDirection.rtl,
                              child: Text(feedback[i]!, style: const TextStyle(fontSize: 13, height: 1.6, color: Colors.white70)),
                            ),
                          ),
                        ],
                      ]),
...
// ============================================================
// MISTAKE REVIEW — 🔁 مرور اشتباهات (فقط سوال‌هایی که غلط زدی)
// ============================================================
...
class _MistakeReviewPageState extends State<MistakeReviewPage> {
  ...
  Future<void> _load() async {
    final all = await MistakeBook.getAll();
    setState(() {
      items = List<MistakeItem>.from(all)..shuffle();
      loading = false;
    });
  }
  ...
  Future<void> _select(String opt) async {
    ...
    if (opt == items[index].answer) {
      await MistakeBook.remove(items[index]);
    }
  }
  ...
  Widget build(BuildContext context) {
    if (loading) {
      return Scaffold(
        appBar: AppBar(backgroundColor: Colors.transparent),
        body: const Stack(children: [NeonBackground(), Center(child: CircularProgressIndicator())]),
      );
    }
    ...
    if (items.isEmpty) {
      return Scaffold(
        appBar: AppBar(
          title: const Directionality(textDirection: TextDirection.rtl, child: Text('مرور اشتباهات')),
          backgroundColor: Colors.transparent,
        ),
        body: Stack(children: [
          const NeonBackground(),
          Center(
            child: Directionality(
              ...
              child: Column(mainAxisAlignment: MainAxisAlignment.center, children: const [
                Text('🎉', style: TextStyle(fontSize: 60)),
                SizedBox(height: 12),
                Text('هیچ اشتباهی نداری، عالیه!', style: TextStyle(fontSize: 15, color: Colors.white70)),
                SizedBox(height: 4),
                Text('وقتی تو کوییز چیزی رو اشتباه بزنی، اینجا جمع میشه', style: TextStyle(fontSize: 12, color: Colors.white38)),
              ]),
            ),
          ),
        ]),
          ...
          Center(
            child: Directionality(
              ...
              child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                const Text('✅', style: TextStyle(fontSize: 60)),
                const SizedBox(height: 12),
                const Text('مرور تموم شد', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                const SizedBox(height: 16),
    ...
    return Scaffold(
      appBar: AppBar(
        title: Directionality(
          textDirection: TextDirection.rtl,
          child: Text('مرور اشتباهات  •  ${index + 1}/${items.length}'),
        ),
        backgroundColor: Colors.transparent,
      ),
      body: Stack(children: [
        ...
        Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            Text(item.text, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, height: 1.5)),
            const SizedBox(height: 20),
            Expanded(
              child: ListView(
                children: item.options.map((opt) {
                  final isSelected = selected == opt;
                  final isCorrect = opt == item.answer;
                  Color borderColor = Colors.white24;
                  Color? fillColor;
                  if (selected != null) {
                    if (isSelected && isCorrect) {
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: Directionality(
                    textDirection: TextDirection.rtl,
                    child: Text(
                      index + 1 >= items.length ? 'پایان' : 'بعدی',
...
// ============================================================
// REVERSE TRANSLATION — 🔄 فارسی → انگلیسی (تایپ آزاد، بدون کلمات آماده)
// ============================================================

class ReverseTranslationPage extends StatefulWidget {
  final Lesson? lesson;
  const ReverseTranslationPage({super.key, this.lesson});
  @override
...
class _ReverseTranslationPageState extends State<ReverseTranslationPage> {
  ...
  void initState() {
    super.initState();
    final source = widget.lesson != null
        ? (List<String>.from(widget.lesson!.examples)..shuffle())
        : (lessons.expand((l) => l.examples).toList()..shuffle());
    final limited = source.length > 10 ? source.sublist(0, 10) : source;
    englishSentences = [];
    persianMeanings = [];
    for (final ex in limited) {
      final parts = ex.split(' — ');
  ...
  void _check() {
    final target = _normalize(englishSentences[round]);
    final given = _normalize(answerCtrl.text);
    setState(() => isCorrect = given.isNotEmpty && given == target);
  }

  void _next() {
    if (round + 1 >= englishSentences.length) {
      if (widget.lesson != null) {
        AppState.instance.markActivityComplete(widget.lesson!.id, 'translation');
        AppState.instance.addReward(20, 5);
      }
      Navigator.pop(context);
      return;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Directionality(
          textDirection: TextDirection.rtl,
          child: Text('ترجمه‌ی معکوس  •  ${round + 1}/${englishSentences.length}'),
        ),
        backgroundColor: Colors.transparent,
      ),
      body: Stack(children: [
        ...
        SingleChildScrollView(
          ...
          child: Column(children: [
            const Directionality(
              textDirection: TextDirection.rtl,
              child: Text('این جمله رو به انگلیسی بنویس:', style: TextStyle(fontSize: 13, color: Colors.white54)),
            ),
            const SizedBox(height: 10),
            Directionality(
              textDirection: TextDirection.rtl,
              child: Text(
                persianMeanings[round],
              ...
              decoration: InputDecoration(
                ...
                border: OutlineInputBorder(
                  ...
                  borderSide: BorderSide(
                    color: isCorrect == null ? Colors.transparent : (isCorrect! ? Colors.greenAccent : Colors.redAccent),
                    width: 1.6,
                  ),
                ),
              ),
            ...
            if (isCorrect == null)
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: _check,
                  style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 15)),
                  child: const Text('بررسی', style: TextStyle(fontWeight: FontWeight.bold)),
                ),
              ),
            if (isCorrect != null) ...[
              Directionality(
                textDirection: TextDirection.rtl,
                child: Text(
                  isCorrect! ? '✅ درست بود!' : '❌ جمله‌ی درست: ${englishSentences[round]}',
                  textAlign: TextAlign.center,
                  style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: isCorrect! ? Colors.greenAccent : Colors.orangeAccent),
                ),
              ),
              const SizedBox(height: 16),
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: Directionality(
                    textDirection: TextDirection.rtl,
                    child: Text(
                      round + 1 >= englishSentences.length ? 'پایان' : 'بعدی',
                      style: const TextStyle(fontWeight: FontWeight.bold),
                    ),
                  ),
                ),
            ...
            const Directionality(
              textDirection: TextDirection.rtl,
              child: Text(
                'نکته: نقطه‌گذاری و بزرگی/کوچکی حروف مهم نیست، فقط خودِ جمله مهمه',
...
// ============================================================
// LISTENING GAP-FILL — 🎧 یه کلمه از جمله رو گوش بده و پیداش کن
// ============================================================

class ListeningGapFillPage extends StatefulWidget {
  final Lesson? lesson;
  const ListeningGapFillPage({super.key, this.lesson});
  @override
  State<ListeningGapFillPage> createState() => _ListeningGapFillPageState();
}

class _GapRound {
  final String sentence;
  final String blankWord;
  final String display; // sentence with the word replaced by ____
  _GapRound({required this.sentence, required this.blankWord, required this.display});
}

class _ListeningGapFillPageState extends State<ListeningGapFillPage> {
  late List<_GapRound> rounds;
  int round = 0;
  final FlutterTts tts = FlutterTts();
  final TextEditingController answerCtrl = TextEditingController();
  bool? isCorrect;
  ...
  void initState() {
    ...
    for (final ex in limited) {
      final parts = ex.split(' — ');
      final sentence = (parts.isNotEmpty ? parts[0] : ex).replaceAll('.', '').trim();
      final words = sentence.split(' ').where((w) => w.trim().isNotEmpty).toList();
      final candidates = words.where((w) => w.length >= 3).toList();
      if (candidates.isEmpty) continue;
      final blank = candidates[Random().nextInt(candidates.length)];
      final display = words.map((w) => w == blank ? '____' : w).join(' ');
      rounds.add(_GapRound(sentence: sentence, blankWord: blank, display: display));
    }
  }
  ...
  void _check() {
    final given = answerCtrl.text.trim().toLowerCase().replaceAll(RegExp(r'[.!?,]'), '');
    final target = rounds[round].blankWord.toLowerCase().replaceAll(RegExp(r'[.!?,]'), '');
    setState(() => isCorrect = given.isNotEmpty && given == target);
  }
  ...
  void _next() {
    if (round + 1 >= rounds.length) {
      if (widget.lesson != null) {
        AppState.instance.markActivityComplete(widget.lesson!.id, 'listening');
        AppState.instance.addReward(20, 5);
      }
      Navigator.pop(context);
      return;
  ...
  Widget build(BuildContext context) {
      ...
      return Scaffold(
        ...
        body: Stack(children: [
          ...
          const Center(
            child: Directionality(
              textDirection: TextDirection.rtl,
              child: Text('برای این درس جمله‌ی مناسبی پیدا نشد', style: TextStyle(color: Colors.white54)),
            ),
          ),
        ]),
      );
    ...
    return Scaffold(
      appBar: AppBar(
        title: Directionality(
          textDirection: TextDirection.rtl,
          child: Text('شنیداری  •  ${round + 1}/${rounds.length}'),
        ),
        backgroundColor: Colors.transparent,
      ),
      body: Stack(children: [
        ...
        SingleChildScrollView(
          ...
          child: Column(children: [
            GestureDetector(
              onTap: () => tts.speak(r.sentence),
              child: Container(
                width: 90,
                height: 90,
                decoration: BoxDecoration(
                  shape: BoxShape.circle,
                  color: const Color(0xFF00C8FF).withOpacity(.16),
                  border: Border.all(color: const Color(0xFF00E5FF), width: 2),
                ),
                child: const Icon(Icons.volume_up_rounded, size: 40, color: Color(0xFF00E5FF)),
              ),
            ),
            const SizedBox(height: 10),
            const Directionality(
              textDirection: TextDirection.rtl,
              child: Text('روی آیکون بزن تا جمله پخش بشه، بعد کلمه‌ی جا افتاده رو بنویس', style: TextStyle(fontSize: 12, color: Colors.white54), textAlign: TextAlign.center),
            ),
            const Directionality(
              textDirection: TextDirection.rtl,
              child: Text('اگه صدا پخش نشد، دو بار کلیک کن', style: TextStyle(fontSize: 11, color: Colors.white38)),
            ),
            const SizedBox(height: 24),
            Text(r.display, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
            const SizedBox(height: 24),
            TextField(
              controller: answerCtrl,
              enabled: isCorrect == null,
            ...
            if (isCorrect != null) ...[
              Directionality(
                textDirection: TextDirection.rtl,
                child: Text(
                  isCorrect! ? '✅ درست بود!' : '❌ کلمه‌ی درست: ${r.blankWord}',
                  style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: isCorrect! ? Colors.greenAccent : Colors.orangeAccent),
                ),
              ),
              const SizedBox(height: 16),
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: Directionality(
                    textDirection: TextDirection.rtl,
                    child: Text(
                      round + 1 >= rounds.length ? 'پایان' : 'بعدی',
                      style: const TextStyle(fontWeight: FontWeight.bold),
                    ),
                  ),
                ),
