...
// shuffled across all of them: the word is only SPOKEN
// (flutter_tts) — never shown in English — and the user must
// pick its Persian meaning from 4 options. Wrong answers are
// recorded in the shared Mistake Notebook.
// ============================================================
import 'dart:math';
...
class _ListeningPracticePageState extends State<ListeningPracticePage> {
  ...
  void _startSession() {
    distractorPool = currentLessons.expand((l) => l.words).toList();
    final pool = List<Word>.from(distractorPool)..shuffle(Random());
    setState(() {
  ...
  void _prepareRound() {
    ...
    final others = distractorPool.where((w) => w.word != current.word).toList()..shuffle(Random());
