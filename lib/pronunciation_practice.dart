...
// comparing what they said to the target sentence.
// ============================================================
import 'dart:math';
...
class _PronunciationItem {
  final String text; // English target sentence
...
class PronunciationPracticePage extends StatefulWidget {
  ...
  const PronunciationPracticePage({super.key, this.lesson, this.lessons});
  @override
...
class _PronunciationPracticePageState extends State<PronunciationPracticePage> {
  ...
  late List<_PronunciationItem> items;
  ...
  void initState() {
    ...
    currentLessons = widget.lessons ?? (widget.lesson != null ? [widget.lesson!] : [lessons.first]);
    _rebuildItems();
    _initSpeech();
  }

  void _rebuildItems() {
    final sentencePool = currentLessons
        .expand((l) => [...l.examples, ...l.negativeExamples, ...l.questionExamples])
        .map((e) {
      final parts = e.split(' — ');
    ...
    items = pickShuffledQuestions(sentencePool, _kRounds);
    index = 0;
    recognized = '';
    score = null;
  }
  ...
  void _next() {
    ...
    if (atLastItem) {
      if (currentLessons.length == 1) {
        AppState.instance.markActivityComplete(currentLessons.first.id, 'pronunciation');
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Directionality(textDirection: TextDirection.rtl, child: Text('تمرین تلفظ جملات')),
