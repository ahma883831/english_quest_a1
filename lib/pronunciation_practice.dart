...
class _PronunciationPracticePageState extends State<PronunciationPracticePage> {
  ...
  void _rebuildItems() {
    final wordPool = currentLessons
        .expand((l) => l.words)
        .map((w) => _PronunciationItem(text: w.word, meaningFa: w.meaning))
        .toList();
    final sentencePool = currentLessons.expand((l) => l.examples).map((e) {
      final parts = e.split(' — ');
      final en = parts.isNotEmpty ? parts[0] : e;
      final fa = parts.length > 1 ? parts[1] : '';
      return _PronunciationItem(text: en, meaningFa: fa);
    }).toList();
    wordItems = pickShuffledQuestions(wordPool, _kRounds);
    sentenceItems = pickShuffledQuestions(sentencePool, _kRounds);
    index = 0;
    recognized = '';
    score = null;
  }
