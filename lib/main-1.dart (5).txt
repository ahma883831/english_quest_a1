  ...
  Widget _practice() => SingleChildScrollView(
        ...
        child: Column(children: [
          ...
          _PracticeOptionCard(
            ...
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ListeningPracticePage(initialLesson: widget.lesson))),
          ),
          const SizedBox(height: 16),
          _PracticeOptionCard(
            emoji: '⌨️',
            ...
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ReverseRecallPage(initialLesson: widget.lesson))),
          ),
          const SizedBox(height: 16),
          _PracticeOptionCard(
            emoji: '📖',
            ...
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ChainStoryPage(initialLesson: widget.lesson))),
          ),
          const SizedBox(height: 16),
          _PracticeOptionCard(
            emoji: '⚡',
...
class PracticeHubPage extends StatelessWidget {
  ...
  Future<void> _pickManyThen(BuildContext context, Widget Function(List<Lesson>) builder) async {
          ...
          SingleChildScrollView(
            ...
            child: Column(children: [
              _PracticeOptionCard(
                ...
                subtitleFa: 'بازی ساخت کلمه از روی معنی و تلفظ',
                color: const Color(0xFF00E5A0),
                onTap: () => _pickManyThen(context, (ls) => WordBuilderPage(lessons: ls)),
              ),
              const SizedBox(height: 16),
              _PracticeOptionCard(
                emoji: '📝',
                ...
                subtitleFa: 'جمله‌ی انگلیسی رو از روی معنی بساز',
                color: const Color(0xFF7C4DFF),
                onTap: () => _pickManyThen(context, (ls) => SentenceBuilderPage(lessons: ls)),
              ),
              const SizedBox(height: 16),
              _PracticeOptionCard(
                emoji: '🗣️',
                ...
                onTap: () => _pickManyThen(context, (ls) => PronunciationPracticePage(lessons: ls)),
              ),
              const SizedBox(height: 16),
              _PracticeOptionCard(
                emoji: '🎧',
                ...
                subtitleFa: 'فقط صدا رو بشنو و معنی رو انتخاب کن',
                color: const Color(0xFF29B6F6),
                onTap: () => _pickManyThen(context, (ls) => ListeningPracticePage(initialLessons: ls)),
              ),
              const SizedBox(height: 16),
              _PracticeOptionCard(
                emoji: '⌨️',
                ...
                subtitleFa: 'معنی فارسی رو ببین، کلمه‌ی انگلیسی رو تایپ کن',
                color: const Color(0xFFFFB300),
                onTap: () => _pickManyThen(context, (ls) => ReverseRecallPage(initialLessons: ls)),
              ),
              const SizedBox(height: 16),
              _PracticeOptionCard(
                emoji: '📖',
                ...
                color: const Color(0xFF66BB6A),
                onTap: () => _pickManyThen(context, (ls) => ChainStoryPage(initialLessons: ls)),
              ),
              const SizedBox(height: 16),
              _PracticeOptionCard(
                emoji: '⚡',
...
class _WordBuilderPageState extends State<WordBuilderPage> {
  ...
  void initState() {
    ...
    tts.setLanguage('en-US');
    tts.setSpeechRate(0.42);
    currentLessons = widget.lessons ?? (widget.lesson != null ? [widget.lesson!] : lessons);
    _rebuildPool();
  }

  void _rebuildPool() {
    final wordPool = currentLessons.expand((l) => l.words).toList();
    pool = pickShuffledQuestions(wordPool, _kRounds);
    round = 0;
    correctCount = 0;
  ...
  Future<void> _changeLesson() async {
    final picked = await Navigator.push<List<Lesson>>(
      context,
      MaterialPageRoute(builder: (_) => MultiLessonPickerPage(initialSelected: currentLessons)),
    );
    if (picked != null && picked.isNotEmpty) {
      setState(() {
...
class SentenceBuilderPage extends StatefulWidget {
  ...
  // Preferred going forward: pre-select several lessons at once.
  final List<Lesson>? lessons;
  const SentenceBuilderPage({super.key, this.lesson, this.lessons});
  @override
...
class _SentenceBuilderPageState extends State<SentenceBuilderPage> {
  ...
  void _rebuildSentences() {
    final allExamples = currentLessons.expand((l) => l.examples).toList();
    final limited = pickShuffledQuestions(allExamples, _kRounds);
    englishSentences = [];
    persianMeanings = [];
